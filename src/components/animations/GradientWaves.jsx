...
const GradientWaves = ({
  ...
  speed = 0.4, amplitude = 2.5, waveScale = 0.6, waveRatio = 0.9, swell = 35,
  turbulence = 20, tilt = 1.11, zoom = 1.0, height = 5.5, fogDepth = 15,
  detail = 'medium', brightness = 1.0, opacity = 1.0, mouseInteraction = true,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      vertex, fragment,
      uniforms: {
        iTime: { value: 0 }, iResolution: { value: new Float32Array([1, 1]) }, uSpeed: { value: 0.4 },
        uAmplitude: { value: 2.5 }, uWaveScale: { value: 0.6 }, uWaveRatio: { value: 0.9 }, uSwell: { value: 35 },
        uTurbulence: { value: 20 }, uTilt: { value: 1.11 }, uZoom: { value: 1.0 }, uHeight: { value: 5.5 },
        uFogDepth: { value: 15 }, uSteps: { value: 70.0 }, uBrightness: { value: 1.0 }, uOpacity: { value: 1.0 },
        uGrain: { value: 1.0 }, uGrainIntensity: { value: 0.05 }, uMouse: { value: new Float32Array([0.5, 0.5]) },
    ...
    const onPointerMove = e => {
      ...
      targetMouse[0] = (e.clientX - rect.left) / rect.width; targetMouse[1] = 1.0 - (e.clientY - rect.top) / rect.height;
    };
    ...
    const loop = t => {
      ...
      currentMouse[0] += 0.05 * (tx - currentMouse[0]); currentMouse[1] += 0.05 * (ty - currentMouse[1]);
      program.uniforms.uMouse.value[0] = currentMouse[0]; program.uniforms.uMouse.value[1] = currentMouse[1];
      renderer.render({ scene: mesh });
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container || !ctxMap.has(container)) return;
    const { program } = ctxMap.get(container);
    ...
    const hc = u.uHorizonColor.value, wc = u.uWaveColor.value, cc = u.uCrestColor.value;
    const h = hexToRgb(horizonColor), w = hexToRgb(waveColor), cr = hexToRgb(crestColor);
